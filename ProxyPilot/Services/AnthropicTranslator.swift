...
final class GoogleThoughtSignatureStore: @unchecked Sendable {
    ...
    init(ttl: TimeInterval = 300, maxEntries: Int = 1_000) {
        self.ttl = ttl
        self.maxEntries = maxEntries
    }

    func store(signature: String, for toolCallID: String) {
    ...
    func lookup(toolCallID: String) -> String? {
...
enum AnthropicTranslator {
    struct TranslationContext: Sendable {
        ...
        init(
            upstreamProvider: AppViewModel.UpstreamProvider = .openAI,
        ...
        var isGoogleGemini3: Bool {
            upstreamProvider == .google && resolvedUpstreamModel.lowercased().hasPrefix("gemini-3")
        }
    }

    struct RequestTranslationResult {
        let payload: [String: Any]
        let injectedGoogleSignatures: Int
        let usedGoogleBypassFallback: Bool
    }

    struct ResponseTranslationResult {
        let payload: [String: Any]
        let capturedGoogleSignatures: Int
    }

    // MARK: - Request: Anthropic → OpenAI

    static func requestToOpenAI(_ anthropic: [String: Any]) -> [String: Any] {
        requestToOpenAI(anthropic, context: TranslationContext()).payload
    }

    static func requestToOpenAI(
    ...
    static func stripUnsupportedParameters(
        _ request: inout [String: Any],
        for provider: AppViewModel.UpstreamProvider
    ) {
    ...
    static func responseFromOpenAI(_ openAI: [String: Any], model: String) -> [String: Any] {
        responseFromOpenAI(openAI, model: model, context: TranslationContext()).payload
    }

    static func responseFromOpenAI(
        _ openAI: [String: Any],
        model: String,
        context: TranslationContext
    ...
    static func streamingStartEvents(messageId: String, model: String) -> [String] {
    ...
    static func streamingTextStartEvent(index: Int) -> String {
    ...
    static func streamingDeltaEvent(index: Int = 0, text: String) -> String {
    ...
    static func streamingToolUseStartEvent(index: Int, id: String, name: String) -> String {
        let contentBlockStart: [String: Any] = [
            "type": "content_block_start",
            "index": index,
            "content_block": [
    ...
    static func streamingToolUseInputDeltaEvent(index: Int, partialJSON: String) -> String {
        let delta: [String: Any] = [
            "type": "content_block_delta",
            "index": index,
            "delta": [
    ...
    static func streamingContentBlockStopEvent(index: Int) -> String {
    ...
    static func streamingMessageDeltaEvent(stopReason: String, outputTokens: Int = 0) -> String {
    ...
    static func streamingMessageStopEvent() -> String {
    ...
    static func streamingDoneEvents(messageId: String, model: String) -> [String] {
        let contentBlockStop: [String: Any] = [
            "type": "content_block_stop",
            "index": 0
        ]
    ...
    struct StreamingState {
        ...
        var sentMessageStart: Bool = false
        var messageID: String
        var nextAnthropicIndex: Int = 0
        var textAnthropicIndex: Int?
        var startedAnthropicIndexes: [Int] = []
        var toolStatesByOpenAIIndex: [Int: ToolState] = [:]
        var toolCallIDsByOpenAIIndex: [Int: String] = [:]
        var capturedGoogleToolCallIDs: Set<String> = []
        var sawToolUse: Bool = false
        var finalStopReason: String = "end_turn"
        var upstreamChunkIndex: Int = 0
        var streamedEventCount: Int = 0
        var lastSeenPromptTokens: Int = 0
        var lastSeenCompletionTokens: Int = 0
    }

    static func processStreamingChunk(
        _ chunk: [String: Any],
    ...
    static func streamingFinishEvents(state: StreamingState) -> [String] {
